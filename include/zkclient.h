...
**/
zoo_rc create_persistent_node(const char* path, const std::string& value, const 
...
**/
zoo_rc create_sequence_node(const char* path, const std::string& value, const std::vector<zoo_acl_t>& 
acl, std::string& returned_path_name);
?
/** 
** the ephemeral node will auto delete if the session is disconnect
**/
zoo_rc create_ephemeral_node(const char* path, const std::string& value, const 
std::vector<zoo_acl_t>& acl);
?
/** 
** @param returned_path_name - return the real path name of the node
**/
zoo_rc create_sequance_ephemeral_node(const char* path, const std::string& value, const 
std::vector<zoo_acl_t>& acl, std::string& returned_path_name);
?
...
** @param
** path - the node name
** out_value - return the data associated with the node
** info - return the state info of the node
** watch - watch if true, a watch will be set at the server to notify
...
** - z_invalid_state : zhandle state is either in {@link #zoo_state_expired_session} or {@link 
#zoo_state_auth_failed}
...
** @param
** path - the node name
** value - the new data to written to the node
...
** - z_bad_arguments : invalid input parameters
** - z_invalid_state : zhandle state is either {@link #zoo_state_expired_session} or {@link 
#zoo_state_auth_failed}
** - z_marshallerror : failed to marshall a request; possibly, out of memory
**/
