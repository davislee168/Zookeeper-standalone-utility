...
void zkconnect::globalWatcher(zhandle_t *p_pZH, int p_iType, int p_iState, const char *p_szPath, void *p_pWatcherCtx)
{
  ...
  // printf("globalWatcher |%s[%d]|%s[%d]\n", Type2String(p_iType), p_iType, State2String(p_iState), p_iState);
...
zoo_rc zkconnect::create_node(const char* path, const string& value, const vector<zoo_acl_t>& acl, int32_t create_flags, char *path_buffer, int32_t path_buffer_len)
{
  ...
  */
  ...
  if (value.empty())
    ...
    rt = (zoo_rc)zoo_create(m_zk, path, value.c_str(), (int)value.size(), &acl_v, create_flags, path_buffer, (int)path_buffer_len);
...
string zkconnect::create(const string &path, const char data[], int mode)
{
  ...
  if (data == NULL)
    ...
    rc = zoo_create(m_zk, path.c_str(), data, sizeof(data)-1, &ZOO_OPEN_ACL_UNSAFE, mode, buffer, sizeof(buffer)-1);

  // zoo_rc rt = (zoo_rc)zoo_create(m_zk, path.c_str(), NULL, 0, &ZOO_OPEN_ACL_UNSAFE, mode, buffer, sizeof(buffer)-1);
 ...
 */
  ...
  if (zookeeper_get_connected_host(m_zk, reinterpret_cast<struct sockaddr*>(&addr_in6), &addr_len) == NULL) {
    printf("Trow exception\n");
  }
